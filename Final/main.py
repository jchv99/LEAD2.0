...
def p_varParam(p):
    '''varParam : ID p_n_nameID p_n_nAccesoNormal
                | ID p_n_nameID LSQUARE CTEINT p_n_nAccesoArreglo RSQUARE'''
...
def p_escrituraTemp(p):
    '''escrituraTemp : varTemp p_n_escrituraExp
                     | expresion p_n_escrituraExp
                     | p_escrituraTemp2 
...
def p_n_asignacion(p):
    '''p_n_asignacion : '''
    global currentVarTemp, currentScope, contVarTemp, contCuadruplos
    asig = pilaO.pop()
    asigType = pTipos.pop()
...
def p_n_addPilaO(p):
    '''p_n_addPilaO : '''
    global pilaO, pTipos, currentScope, currentVar, nAcceso
...
while(contTempQuads != contCuadruplos+1):
    ...
    #################################
        ...
        if res >= 3000 and res < 4000:
            arrLocalInt[res - 6000 + basInt[-1]] = input()
        
        elif res >= 4000 and res < 5000:
            arrLocalFloat[res-8000 + basFloat[-1]] = input()

        elif res >= 5000 and res < 6000:
            arrLocalChar[res-10000 + basChar[-1]] = input()

        elif res >= 7000 and res < 8000:
            arrGlobalInt[res-7000] = input()
        
        elif res >= 8000 and res < 9000:
            arrGlobalFloat[res-8000] = input()
